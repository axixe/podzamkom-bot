...
def employee_manager_keyboard(period: str) -> InlineKeyboardMarkup:
    rows = [
        ...
        [InlineKeyboardButton("Список сотрудников", callback_data="manager_list:0")],
...
def employee_list_keyboard(items: list[tuple[int, str]], page: int, has_prev: bool, has_next: bool) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(title, callback_data=f"manager_employee:{employee_id}:day")] for employee_id, title in items]

    nav_row = []
    if has_prev:
        nav_row.append(InlineKeyboardButton("⬅️", callback_data=f"manager_list:{page - 1}"))
    if has_next:
        nav_row.append(InlineKeyboardButton("➡️", callback_data=f"manager_list:{page + 1}"))
    if nav_row:
        rows.append(nav_row)
