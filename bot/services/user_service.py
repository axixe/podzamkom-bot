...
from bot.services.admin_service import notify_admin_new_photos
from bot.services.employee_service import format_employee_display, get_allowed_employee
...
async def submit_user_photos(query, context: ContextTypes.DEFAULT_TYPE, store: DataStore) -> None:
    ...
    employee = get_allowed_employee(store, user.id, user.username)
    if not employee:
    ...
    await notify_admin_new_photos(
        context=context,
        from_employee_title=format_employee_display(employee),
        count=added_count,
        store=store,
    )
